...
public class AbstractSessionStrategy {
    ...
    public AbstractSessionStrategy(SecurityProperties securityPropertie) {
        String invalidSessionUrl = securityPropertie.getClientProperties().getSession().getSessionInvalidUrl();
    ...
    protected void onSessionInvalid(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (StringUtils.endsWithIgnoreCase(sourceUrl, ".html")) {
            if(StringUtils.equals(sourceUrl, securityPropertie.getClientProperties().getSignInPage())
                    || StringUtils.equals(sourceUrl, securityPropertie.getClientProperties().getSignOutUrl())){
                targetUrl = sourceUrl;
            }else{
...
//        if (UserAgentUtils.isMobile(request) || Servlets.isAjaxRequest(request)) {
//            Object result = buildResponseContent(request);
//            response.setStatus(HttpStatus.UNAUTHORIZED.value());
//            response.setContentType("application/json;charset=UTF-8");
//            response.getWriter().write(objectMapper.writeValueAsString(result));
